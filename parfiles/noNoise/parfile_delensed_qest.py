...
TEMP =  os.path.join(os.environ['PLENS'], 'noNoise_ideal_delensed_qest')
...
lmax_qlm = 2048 # We will calculate lensing estimates until multipole lmax_qlm.
...
ftl = utils.cli(cl_unl['tt'][:lmax_ivf + 1] + (nlev_t / 60. / 180. * np.pi / transf) ** 2)
fel = utils.cli(cl_unl['ee'][:lmax_ivf + 1] + (nlev_p / 60. / 180. * np.pi / transf) ** 2)
fbl = utils.cli(cl_unl['bb'][:lmax_ivf + 1] + (nlev_p / 60. / 180. * np.pi / transf) ** 2)
ftl[:lmin_ivf] *= 0.
fel[:lmin_ivf] *= 0.
fbl[:lmin_ivf] *= 0.
...
qresp_dd = qresp.resp_lib_simple(os.path.join(TEMP, 'qresp_delensed'), lmax_ivf, cl_weight, cl_unl,
