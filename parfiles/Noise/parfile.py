...
TEMP =  os.path.join(os.environ['PLENS'], 'noisy_ivfs_lensed')
...
# Centralized map sims
sims  = planck2018_sims.smica_dx12_custom(include_noise=True, sims_dcl=sims_dcl)
...
nl_TT =  (nlev_t / 60. / 180. * np.pi / transf) ** 2
ftl = cl_len['tt'][:lmax_ivf+1] / (cl_len['tt'][:lmax_ivf+1] + nl_TT[:lmax_ivf+1])
qcls_pt = ptcls(os.path.join(TEMP, 'qcls_pt'), qlms_dd, ivfs_raw, mc_sims_mf=mc_sims_mf_dd, ftl=ftl)
