...
def get_blbubc(bin_type):
    if "full_" in bin_type:
        stepsize = int(bin_type.split('_')[1])
        stepstart = 2
        stepend = 2048
        bins_l = np.arange(stepstart, stepend - stepsize, stepsize)
        bins_u = np.arange(stepstart + stepsize - 1, stepend - 1, stepsize)
        # print(bins_l)
        # print(bins_u)

    elif "fullA_" in bin_type:
        stepsize = int(bin_type.split('_')[1])
        stepstart = 5
        stepend = 600
        bins_l = np.arange(stepstart, stepend - stepsize, stepsize)
